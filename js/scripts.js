    ...
    ***********************************************/
    $('#index-subs').validate({
      ...
      errorPlacement: function(error, element) {
        if ( element.parent('.checkbox, .radio, .switcher').length )
          error.insertAfter( element.parent('.checkbox, .radio, .switcher') );
        else if ( element.hasClass('chosen-select') || element.hasClass('chosen-select-no-single') )
      ...
      submitHandler: function(form, event) {
        ...
        $.ajax({
          type: "POST",
          url: "subscribe/request.php",
          data: formData,
          success: function() {
            // $(form).find('.show--success').show();
            // $(form).find('.hide--success').hide();
            document.location.href = 'http://solaris.online/index2.html';
            setTimeout(function() {
    ...
    $('#index-subs').on('click','.form_revert-link',function(event) {
      event.preventDefault();
      var form = $('#index-subs');
      form.find('.show--success').hide();
      form.find('.hide--success').show();
